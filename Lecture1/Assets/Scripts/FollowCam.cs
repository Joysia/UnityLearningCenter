...
public class FollowCam : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        if (camDist <= dist) tr.position = Vector3.Lerp(tr.position, targetTr.position - (targetTr.forward * camDist) + (Vector3.up * height), Time.deltaTime * dampTrace);
        else tr.position = Vector3.Lerp(tr.position, targetTr.position - (targetTr.forward * dist) + (Vector3.up * height), Time.deltaTime * dampTrace);
    }
}