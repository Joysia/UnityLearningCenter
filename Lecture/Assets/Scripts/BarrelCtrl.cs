...
public class BarrelCtrl : MonoBehaviour
{
    ...
    private Collider[] colls;                       // 충돌체값을 공용화
    private List<Rigidbody> LRigidbody;                // 충돌체중
    private List<Transform> LTransform;
    private ArrayList ACollider;
    ...
    private void OnCollisionEnter(Collision coll)
    {
        if (coll.collider.tag == "Bullet")
        {
            ...
            GameMgr.instance.PlaySfx(transform.position, BarrelBoomSound);
            //BarrelAS.Play();
            //}
        }
    }
    ...
    {
        // _params[0] 에 스파크 생성하고 Destoy 하는 코드를 넣을 것.

        //GameObject spark = (GameObject)Instantiate(spark,_params[0])

        //if(++hitCount >= 3)
        //{
        Vector3 hitPos = (Vector3)_params[0];
        Vector3 firePos = (Vector3)_params[1];
        Vector3 incomVector = hitPos - firePos;  // 입사각 = 맞은 위치 - 발사 원점
        GetComponent<Rigidbody>().AddForceAtPosition(incomVector.normalized * 1000.0f, hitPos);
